...
Carousel.prototype = {
    ...
    createDOM:function(container,options){
        ...
        for(var i = 0;i<imgLength;i++){
            ...
            $(".transverse-box").append("<a class='img-item' href='"+options.imgurl[i]+"' target='_blank'><img src='"+options.datas[i]+"'></a>");
        };

        $(".vertical-box ul").append("<li><a href='"+options.imgurl[1]+"' target='_blank'><img src='"+options.datas[1]+"'></a></li>");
        $(".vertical-box ul").append("<li><a href='"+options.imgurl[2]+"' target='_blank'><img src='"+options.datas[2]+"'></a></li>");
    ...
    changeZindex_add:function(){
        ...
        var firstImgHref =  $(".transverse-box").find("a").attr("href");
        ...
        var lastHrefSrc =  $(".transverse-box").find("a").eq(length-1).attr("href");
        var last2HrefSrc = $(".transverse-box").find("a").eq(length-2).attr("href");

        $(".transverse-box").find(".img-item").eq(0).remove();

        $(".transverse-box").append("<a class='img-item' zindex='"+(parseInt(lastZindex)+1)+"' href='"+firstImgHref+"' target='_blank'><img src='"+firstImgSrc+"'></a>");

        $(".vertical-box ul").find("li").eq(0).find("img").attr("src",lastImgSrc);
        $(".vertical-box ul").find("li").eq(1).find("img").attr("src",last2ImgSrc);

        $(".vertical-box ul").find("li").eq(0).find("a").attr("href",lastHrefSrc);
        $(".vertical-box ul").find("li").eq(1).find("a").attr("href",last2HrefSrc);
    ...
    changeZindex_sub:function(){
        ...
        var lastImgHref = $(".transverse-box").find("a").eq(length-1).attr("href");
        ...
        var firstHrefSrc =  $(".transverse-box").find("a").eq(0).attr("href");
        var first2HrefSrc = $(".transverse-box").find("a").eq(1).attr("href");


        $(".transverse-box").find(".img-item").eq(length-1).remove();

        $(".transverse-box").prepend("<a class='img-item' zindex='"+(parseInt(firstZindex)-1)+"' href='"+lastImgHref+"' target='_blank'><img src='"+lastImgSrc+"'></a>");

        $(".vertical-box ul").find("li").eq(0).find("img").attr("src",firstImgSrc);
        $(".vertical-box ul").find("li").eq(1).find("img").attr("src",first2ImgSrc);

        $(".vertical-box ul").find("li").eq(0).find("a").attr("href",firstHrefSrc);
        $(".vertical-box ul").find("li").eq(1).find("a").attr("href",first2HrefSrc);
